...
create or replace temp macro result_set_part2 (no_of_rules) as table
with recursive moving (
    rec_no,
    stack_one_original,
    stack_one_changed,
    ...
    select 
        ...
        case 
            when move_to = '1'
            then list_concat( stack_one_original, removed_items )
            else stack_one_changed
        end,
        stack_two_original,
        case 
            when move_to = '2'
            then list_concat( stack_two_original, removed_items )
            else stack_two_changed
        end,
        stack_three_original,
        case 
            when move_to = '3'
            then list_concat( stack_three_original, removed_items )
            else stack_three_changed
        end,
        stack_four_original,
        case 
            when move_to = '4'
            then list_concat( stack_four_original, removed_items )
            else stack_four_changed
        end,
        stack_five_original,
        case 
            when move_to = '5'
            then list_concat( stack_five_original, removed_items )
            else stack_five_changed
        end,
        stack_six_original,
        case 
            when move_to = '6'
            then list_concat( stack_six_original, removed_items )
            else stack_six_changed
        end,
        stack_seven_original,
        case 
            when move_to = '7'
            then list_concat( stack_seven_original, removed_items )
            else stack_seven_changed
        end,
        stack_eight_original,
        case 
            when move_to = '8'
            then list_concat( stack_eight_original, removed_items )
            else stack_eight_changed
        end,
        stack_nine_original,
        case 
            when move_to = '9'
            then list_concat( stack_nine_original, removed_items )
            else stack_nine_changed
        end,
        removed_items
    from
    ...
    select 
        ...
        case
            when move_to = '1'
            THEN list_concat( stack_one_changed, removed_items)
            else stack_one_changed
        end,
        stack_two_original,
        case
            when move_to = '2'
            THEN list_concat( stack_two_changed, removed_items )
            else stack_two_changed
        end,
        stack_three_original,
        case
            when move_to = '3'
            THEN list_concat( stack_three_changed, removed_items )
            else stack_three_changed
        end,
        stack_four_original,
        case
            when move_to = '4'
            THEN list_concat( stack_four_changed, removed_items)
            else stack_four_changed
        end,
        stack_five_original,
        case
            when move_to = '5'
            THEN list_concat( stack_five_changed, removed_items )
            else stack_five_changed
        end,
        stack_six_original,
        case
            when move_to = '6'
            THEN list_concat( stack_six_changed, removed_items)
            else stack_six_changed
        end,
        stack_seven_original,
        case
            when move_to = '7'
            THEN list_concat( stack_seven_changed, removed_items )
            else stack_seven_changed
        end,
        stack_eight_original,
        case
            when move_to = '8'
            THEN list_concat( stack_eight_changed, removed_items)
            else stack_eight_changed
        end,
        stack_nine_original,
        case
            when move_to = '9'
            THEN list_concat( stack_nine_changed, removed_items )
            else stack_nine_changed
        end,
        removed_items
    from 
...
select * from result_set_part2(504);
